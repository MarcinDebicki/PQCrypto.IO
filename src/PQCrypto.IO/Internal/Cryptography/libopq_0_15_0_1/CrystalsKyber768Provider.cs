...
internal sealed class CrystalsKyber768Provider : IKeyEncapsulationProvider
{
    ...
    private static readonly int OQS_KEM_KYBER_768_LENGTH_CIPHERTEXT = 1088;
    private static readonly int OQS_KEM_KYBER_768_LENGTH_PUBLIC_KEY = 1184;
    private static readonly int OQS_KEM_KYBER_768_LENGTH_SECRET_KEY = 2400;
    private static readonly int OQS_KEM_KYBER_768_LENGTH_SHARED_SECRET = 32;

    public KeyEncapsulationAlgorithm KeyEncapsulationAlgorithm { get; } = KeyEncapsulationAlgorithm.CrystalsKyber768;
    public LibVersion LibVersion { get; } = LibVersion.libopq_0_15_0_1;

    static CrystalsKyber768Provider()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            DecapsMethod = Windows.OQS_KEM_ml_kem_768_decaps;
            EncapsMethod = Windows.OQS_KEM_ml_kem_768_encaps;
            GenerateKeypairMethod = Windows.OQS_KEM_ml_kem_768_keypair;
        ...
        {
            DecapsMethod = Linux.OQS_KEM_ml_kem_768_decaps;
            EncapsMethod = Linux.OQS_KEM_ml_kem_768_encaps;
            GenerateKeypairMethod = Linux.OQS_KEM_ml_kem_768_keypair;
    ...
    public IKeyEncapsulationResult Decapsulation(in IKeyEncapsulationCiphertext keyEncapsulationCiphertext, in IKeyEncapsulationPrivateKey keyEncapsulationPrivateKey)
    {
        ...
        keyEncapsulationCiphertext.Value.RequireExactLength(nameof(keyEncapsulationCiphertext), OQS_KEM_KYBER_768_LENGTH_CIPHERTEXT);
        keyEncapsulationPrivateKey.Value.RequireExactLength(nameof(keyEncapsulationPrivateKey), OQS_KEM_KYBER_768_LENGTH_SECRET_KEY);

        var plainSessionKeyMemorySafe = ProtectMemoryManager.Instance.Rent(OQS_KEM_KYBER_768_LENGTH_SHARED_SECRET);
    ...
    public IKeyEncapsulationResult Encapsulation(in IKeyEncapsulationPublicKey publicKey)
    {
        ...
        publicKey.Value.RequireExactLength(nameof(publicKey), OQS_KEM_KYBER_768_LENGTH_PUBLIC_KEY);

        var secretSessionKey = new byte[OQS_KEM_KYBER_768_LENGTH_CIPHERTEXT];
        var plainSessionKeyMemorySafe = ProtectMemoryManager.Instance.Rent(OQS_KEM_KYBER_768_LENGTH_SHARED_SECRET);
        var apiResult = -1;
    ...
    {
        var publicKey = new byte[OQS_KEM_KYBER_768_LENGTH_PUBLIC_KEY];
        var privateKeyMemorySafe = ProtectMemoryManager.Instance.Rent(OQS_KEM_KYBER_768_LENGTH_SECRET_KEY);
        var apiResult = -1;
    ...
    private static class Linux
    {
        ...
        internal static extern int OQS_KEM_ml_kem_768_decaps(IntPtr plainSessionKey, byte[] secretSessionKey, IntPtr privateKey);

        [DllImport(NativeLibraryPath.LINUX_PATH_0_15_0_1, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_KEM_ml_kem_768_encaps(byte[] secretSessionKey, IntPtr plainSessionKey, byte[] publicKey);

        [DllImport(NativeLibraryPath.LINUX_PATH_0_15_0_1, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_KEM_ml_kem_768_keypair(byte[] publicKey, IntPtr privateKey);
    }
