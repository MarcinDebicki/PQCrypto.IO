...
internal sealed class SphincsPlusSha2128fProvider : IDigitalSignatureProvider
{
    private static readonly GenerateKeypair GenerateKeypairMethod;

    private static readonly int OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_BYTES = 17088;
    private static readonly int OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES = 32;
    private static readonly int OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES = 64;

    private static readonly Sign SignMethod;
    private static readonly Verify VerifyMethod;

    public DigitalSignatureAlgorithm DigitalSignatureAlgorithm { get; } = DigitalSignatureAlgorithm.SphincsPlusSha2128f;
    public LibVersion LibVersion { get; } = LibVersion.libopq_0_15_0_1;

    static SphincsPlusSha2128fProvider()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            GenerateKeypairMethod = Windows.OQS_SIG_sphincs_sha2_128f_simple_keypair;
            SignMethod = Windows.OQS_SIG_sphincs_sha2_128f_simple_sign;
            VerifyMethod = Windows.OQS_SIG_sphincs_sha2_128f_simple_verify;
        ...
        {
            GenerateKeypairMethod = Linux.OQS_SIG_sphincs_sha2_128f_simple_keypair;
            SignMethod = Linux.OQS_SIG_sphincs_sha2_128f_simple_sign;
            VerifyMethod = Linux.OQS_SIG_sphincs_sha2_128f_simple_verify;
    ...
    private static class Linux
    {
        ...
        internal static extern int OQS_SIG_sphincs_sha2_128f_simple_keypair(byte[] publicKey, IntPtr secretKey);

        [DllImport(NativeLibraryPath.LINUX_PATH_0_15_0_1, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_SIG_sphincs_sha2_128f_simple_sign(byte[] signature, ref nuint signatureLen, byte[] message, nuint messageLen, IntPtr secretKey);

        [DllImport(NativeLibraryPath.LINUX_PATH_0_15_0_1, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_SIG_sphincs_sha2_128f_simple_verify(byte[] message, nuint messageLen, byte[] signature, nuint signatureLen, byte[] publicKey);
    }
