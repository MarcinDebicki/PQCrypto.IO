...
internal sealed class CrystalsDilithium3Provider : IDigitalSignatureProvider
{
    private static readonly GenerateKeypair GenerateKeypairMethod;

    private static readonly int OQS_SIG_DILITHIUM_3_LENGTH_PUBLIC_KEY = 1952;
    private static readonly int OQS_SIG_DILITHIUM_3_LENGTH_SECRET_KEY = 4032;
    private static readonly int OQS_SIG_DILITHIUM_3_LENGTH_SIGNATURE = 3309;

    private static readonly Sign SignMethod;
    private static readonly Verify VerifyMethod;

    public DigitalSignatureAlgorithm DigitalSignatureAlgorithm { get; } = DigitalSignatureAlgorithm.CrystalsDilithium3;
    public LibVersion LibVersion { get; } = LibVersion.libopq_0_15_0_1;

    static CrystalsDilithium3Provider()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            GenerateKeypairMethod = Windows.OQS_SIG_ml_dsa_65_keypair;
            SignMethod = Windows.OQS_SIG_ml_dsa_65_sign;
            VerifyMethod = Windows.OQS_SIG_ml_dsa_65_verify;
        ...
        {
            GenerateKeypairMethod = Linux.OQS_SIG_ml_dsa_65_keypair;
            SignMethod = Linux.OQS_SIG_ml_dsa_65_sign;
            VerifyMethod = Linux.OQS_SIG_ml_dsa_65_verify;
    ...
    public IDigitalSignature Sign(in IMessage message, in IDigitalSignaturePrivateKey privateKey)
    {
        ...
        privateKey.Value.RequireExactLength(nameof(privateKey), OQS_SIG_DILITHIUM_3_LENGTH_SECRET_KEY);
    ...
    public bool Verify(in IMessage message, in IDigitalSignature digitalSignature, in IDigitalSignaturePublicKey publicKey)
    {
        ...
        digitalSignature.Value.RequireExactLength(nameof(digitalSignature), OQS_SIG_DILITHIUM_3_LENGTH_SIGNATURE);
        publicKey.Value.RequireExactLength(nameof(publicKey), OQS_SIG_DILITHIUM_3_LENGTH_PUBLIC_KEY);
