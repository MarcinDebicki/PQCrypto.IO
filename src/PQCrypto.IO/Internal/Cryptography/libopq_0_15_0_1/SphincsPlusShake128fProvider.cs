...
internal sealed class SphincsPlusShake128fProvider : IDigitalSignatureProvider
{
    ...
    public IDigitalSignatureKeyPair GenerateKeyPair()
    {
        ...
        var privateKeyMemorySafe = ProtectMemoryManager.Instance.Rent(OQS_SIG_SPHINCSSHAKE192_F_SIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES);
        var apiResult = -1;
    ...
    private static class Linux
    {
        ...
        internal static extern int OQS_SIG_sphincs_shake_128f_simple_keypair(byte[] publicKey, IntPtr secretKey);

        [DllImport(NativeLibraryPath.LINUX_PATH_0_15_0_1, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_SIG_sphincs_shake_128f_simple_sign(byte[] signature, ref nuint signatureLen, byte[] message, nuint messageLen, IntPtr secretKey);
