...
internal sealed class SphincsPlusSha2192fProvider : IDigitalSignatureProvider
{
    private static readonly GenerateKeypair GenerateKeypairMethod;

    private static readonly int OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_BYTES = 35664;
    private static readonly int OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES = 48;
    private static readonly int OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES = 96;

    private static readonly Sign SignMethod;
    private static readonly Verify VerifyMethod;

    public DigitalSignatureAlgorithm DigitalSignatureAlgorithm { get; } = DigitalSignatureAlgorithm.SphincsPlusSha2192f;
    public LibVersion LibVersion { get; } = LibVersion.libopq_0_15_0_1;

    static SphincsPlusSha2192fProvider()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            GenerateKeypairMethod = Windows.OQS_SIG_sphincs_sha2_192f_simple_keypair;
            SignMethod = Windows.OQS_SIG_sphincs_sha2_192f_simple_sign;
            VerifyMethod = Windows.OQS_SIG_sphincs_sha2_192f_simple_verify;
        ...
        {
            GenerateKeypairMethod = Linux.OQS_SIG_sphincs_sha2_192f_simple_keypair;
            SignMethod = Linux.OQS_SIG_sphincs_sha2_192f_simple_sign;
            VerifyMethod = Linux.OQS_SIG_sphincs_sha2_192f_simple_verify;
    ...
    {
        var publicKey = new byte[OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES];
        var privateKeyMemorySafe = ProtectMemoryManager.Instance.Rent(OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES);
        var apiResult = -1;
    ...
    public IDigitalSignature Sign(in IMessage message, in IDigitalSignaturePrivateKey privateKey)
    {
        ...
        privateKey.Value.RequireExactLength(nameof(privateKey), OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES);

        var signature = new byte[OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_BYTES];
    ...
    public bool Verify(in IMessage message, in IDigitalSignature digitalSignature, in IDigitalSignaturePublicKey publicKey)
    {
        ...
        digitalSignature.Value.RequireExactLength(nameof(digitalSignature), OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_BYTES);
        publicKey.Value.RequireExactLength(nameof(publicKey), OQS_SIG_SPHINCSSHA2192_F_SIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES);
    ...
    private static class Linux
    {
        ...
        internal static extern int OQS_SIG_sphincs_sha2_192f_simple_keypair(byte[] publicKey, IntPtr secretKey);

        [DllImport(NativeLibraryPath.LINUX_PATH_0_15_0_1, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_SIG_sphincs_sha2_192f_simple_sign(byte[] signature, ref nuint signatureLen, byte[] message, nuint messageLen, IntPtr secretKey);

        [DllImport(NativeLibraryPath.LINUX_PATH_0_15_0_1, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_SIG_sphincs_sha2_192f_simple_verify(byte[] message, nuint messageLen, byte[] signature, nuint signatureLen, byte[] publicKey);
    }
