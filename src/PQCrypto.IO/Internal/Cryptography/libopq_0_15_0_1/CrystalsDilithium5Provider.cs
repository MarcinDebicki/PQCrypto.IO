...
internal sealed class CrystalsDilithium5Provider : IDigitalSignatureProvider
{
    private static readonly GenerateKeypair GenerateKeypairMethod;

    private static readonly int OQS_SIG_DILITHIUM_5_LENGTH_PUBLIC_KEY = 2592;
    private static readonly int OQS_SIG_DILITHIUM_5_LENGTH_SECRET_KEY = 4896;
    private static readonly int OQS_SIG_DILITHIUM_5_LENGTH_SIGNATURE = 4627;

    private static readonly Sign SignMethod;
    private static readonly Verify VerifyMethod;

    public DigitalSignatureAlgorithm DigitalSignatureAlgorithm { get; } = DigitalSignatureAlgorithm.CrystalsDilithium5;
    public LibVersion LibVersion { get; } = LibVersion.libopq_0_15_0_1;

    static CrystalsDilithium5Provider()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            GenerateKeypairMethod = Windows.OQS_SIG_ml_dsa_87_keypair;
            SignMethod = Windows.OQS_SIG_ml_dsa_87_sign;
            VerifyMethod = Windows.OQS_SIG_ml_dsa_87_verify;
        ...
        {
            GenerateKeypairMethod = Linux.OQS_SIG_ml_dsa_87_keypair;
            SignMethod = Linux.OQS_SIG_ml_dsa_87_sign;
            VerifyMethod = Linux.OQS_SIG_ml_dsa_87_verify;
    ...
    {
        var publicKey = new byte[OQS_SIG_DILITHIUM_5_LENGTH_PUBLIC_KEY];
        var privateKeyMemorySafe = ProtectMemoryManager.Instance.Rent(OQS_SIG_DILITHIUM_5_LENGTH_SECRET_KEY);
        var apiResult = -1;
    ...
    public IDigitalSignature Sign(in IMessage message, in IDigitalSignaturePrivateKey privateKey)
    {
        ...
        privateKey.Value.RequireExactLength(nameof(privateKey), OQS_SIG_DILITHIUM_5_LENGTH_SECRET_KEY);

        var signature = new byte[OQS_SIG_DILITHIUM_5_LENGTH_SIGNATURE];
    ...
    public bool Verify(in IMessage message, in IDigitalSignature digitalSignature, in IDigitalSignaturePublicKey publicKey)
    {
        ...
        digitalSignature.Value.RequireExactLength(nameof(digitalSignature), OQS_SIG_DILITHIUM_5_LENGTH_SIGNATURE);
        publicKey.Value.RequireExactLength(nameof(publicKey), OQS_SIG_DILITHIUM_5_LENGTH_PUBLIC_KEY);
    ...
    private static class Linux
    {
        ...
        internal static extern int OQS_SIG_ml_dsa_87_keypair(byte[] publicKey, IntPtr secretKey);

        [DllImport(NativeLibraryPath.LINUX_PATH_0_15_0_1, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_SIG_ml_dsa_87_sign(byte[] signature, ref nuint signatureLen, byte[] message, nuint messageLen, IntPtr secretKey);

        [DllImport(NativeLibraryPath.LINUX_PATH_0_15_0_1, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_SIG_ml_dsa_87_verify(byte[] message, nuint messageLen, byte[] signature, nuint signatureLen, byte[] publicKey);
    }
