...
internal sealed class Falcon512Provider : IDigitalSignatureProvider
{
    private static readonly GenerateKeypair GenerateKeypairMethod;

    private static readonly int OQS_SIG_FALCON_CRYPTO_BYTES = 752;
    ...
    public DigitalSignatureAlgorithm DigitalSignatureAlgorithm { get; } = DigitalSignatureAlgorithm.Falcon512;

    static Falcon512Provider()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            GenerateKeypairMethod = Windows.OQS_SIG_falcon_512_keypair;
            SignMethod = Windows.OQS_SIG_falcon_512_sign;
            VerifyMethod = Windows.OQS_SIG_falcon_512_verify;
        ...
        {
            GenerateKeypairMethod = Linux.OQS_SIG_falcon_512_keypair;
            SignMethod = Linux.OQS_SIG_falcon_512_sign;
            VerifyMethod = Linux.OQS_SIG_falcon_512_verify;
    ...
    public IDigitalSignature Sign(in IMessage message, in IDigitalSignaturePrivateKey privateKey)
    {
        ...
        var result = new DigitalSignature(signatureResult);
    ...
    private static class Linux
    {
        ...
        internal static extern int OQS_SIG_falcon_512_keypair(byte[] publicKey, byte[] secretKey);

        [DllImport(NativeLibraryPath.LINUX_PATH, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_SIG_falcon_512_sign(byte[] signature, ref nuint signatureLen, byte[] message, nuint messageLen, byte[] secretKey);

        [DllImport(NativeLibraryPath.LINUX_PATH, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int OQS_SIG_falcon_512_verify(byte[] message, nuint messageLen, byte[] signature, nuint signatureLen, byte[] publicKey);
    }
